    ...
    public void ApplyFalloff(ref Texture2D tex, Configuration config) {
        for (var i = 0; i <= tex.width; i++) {
            for (var j = 0; j <= tex.height; j++) {
                ...
                horEval = i >= tex.width / 2 ? config.falloffConfig.falloffCurve.Evaluate(horEval) : config.falloffConfig.falloffCurve.Evaluate(1f - horEval);
                vertEval = j >= tex.height / 2 ? config.falloffConfig.falloffCurve.Evaluate(vertEval) : config.falloffConfig.falloffCurve.Evaluate(1f - vertEval);
